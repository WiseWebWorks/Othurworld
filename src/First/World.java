	...
	static void update() {
		...
		if(Main.INSTANCE.save.area.equals("Ortni Town")) {//Ortni Town
			...
			event.addAll(Arrays.asList(
				...
				new Event(270,290,7,12,true, new String[]{"It's a flower.","It doesn't seem very interesting..."},0),
				new Event(291, 251, 10, 5, true, 0),//turtle3
				new Event(33,335,25,15,true,new String[]{"'meow'","(it's an incredibly clean cat)"},0),
				new Event(34,336,23,13,true,0),
				new Event(200,0,20,20,true,"OrtniPath", 0, 0)
			));
			if (Main.INSTANCE.sOEvents == GameSequence.Start) {
				event.addAll(Arrays.asList(
				new Event(82,294,16,31,true,0),//the old dude
				new Event(81,293,18,33,false,new String[]{"'Hello.'","'You must be bursting to get out \nof here, aren't you?'", "'Well, tell you what,I'll help you.'","I've gotten bored of lording over \nthe dust motes in the shack most \ncall my house.'", "'Anyway, all you need to do is \nrun.'","'Again, when I scream and send \nthe men down here, you should \nflee to the giant waterbuffalo'", "'in the sky.'", "'AAUUGHHH! \nMY KIDNEY IS DYING!'\n('flee! go, flee!')"},3),//the old dude's speech
				new Event(171,20,51,40,true,0),//the two gaurds blocking the path
				new Event(170,20,25,42,false, new String[]{"'Hey, Kid.'","'You ain't allowed near this.'"},0),//the left gaurd's talk
				new Event(200,20,25,42,false, new String[]{"'Hello, Child.'","'I am sorry, but you are not \nallowed past.'"},0) //the left gaurd's talk
			));
			}else if(Main.INSTANCE.sOEvents == GameSequence.OldDudeDies) {
				event.addAll(Arrays.asList(
				new Event(81,293,18,33,false,new String[]{"('Flee!')","'AAUUUGGH!'"},0),
				new Event(171,20,51,40,true,3),//the two gaurds blocking the path
				new Event(170,20,25,42,false, new String[]{"'What in  tarnation was that?'","'Don't you go anywhere.'"},3),//the left gaurd's talk
				new Event(200,20,25,42,false, new String[]{"'Was that a scream I heard?'","'Please do not leave.'"},3) //the left gaurd's talk				
				));
			}
